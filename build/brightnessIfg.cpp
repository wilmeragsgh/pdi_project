...
using namespace cv;
// [[Rcpp::export]]
RcppExport SEXP brightnessI(SEXP f1, int brightness) {
    ...
    for(i=0;i<height;i++){   
        for(j=0;j<width;j++){
            for(k=0;k<channels;k++){
                data[i*step+j*channels+k]+= brightness;
                if(data[i*step+j*channels+k] > 255) data[i*step+j*channels+k] = 255;
                if(data[i*step+j*channels+k] < 0) data[i*step+j*channels+k] = 0;
            }
        }
    }
