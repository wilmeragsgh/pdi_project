...
using namespace cv;
// [[Rcpp::export]]
RcppExport SEXP umbralizationI(SEXP f1, int min, int max) {
...
#pragma omp parallel for collapse(3)
    for(i=0;i<height;i++){   
        for(j=0;j<width;j++){
            for(k=0;k<channels;k++){
                if(data[i*step+j*channels+k] > (unsigned char)(max)) data[i*step+j*channels+k] = (unsigned char)(max);
                if(data[i*step+j*channels+k] < (unsigned char)(min)) data[i*step+j*channels+k] = (unsigned char)(min);
            }
        }
    }
    ...
    std::string transformName ("_umbralization");
